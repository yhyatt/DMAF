...
def create_social_preview(output_path: Path) -> None:
    ...
    # Top-left bracket
    draw.line(
        [(bx, by + bracket_size), (bx, by), (bx + bracket_size, by)],
        fill=bracket_color,
        width=bracket_width,
    )
    # Top-right bracket
    draw.line(
        [(bx + bw - bracket_size, by), (bx + bw, by), (bx + bw, by + bracket_size)],
        fill=bracket_color,
        width=bracket_width,
    )
    # Bottom-left bracket
    draw.line(
        [(bx, by + bh - bracket_size), (bx, by + bh), (bx + bracket_size, by + bh)],
        fill=bracket_color,
        width=bracket_width,
    )
    # Bottom-right bracket
    draw.line(
        [(bx + bw - bracket_size, by + bh), (bx + bw, by + bh), (bx + bw, by + bh - bracket_size)],
        fill=bracket_color,
        width=bracket_width,
    )
    ...
    for eye_x in [face_center_x - 40, face_center_x + 40]:
        draw.ellipse([(eye_x - 15, eye_y - 15), (eye_x + 15, eye_y + 15)], fill=(255, 255, 255))
        draw.ellipse([(eye_x - 6, eye_y - 6), (eye_x + 6, eye_y + 6)], fill=(51, 51, 51))

    # Smile
    smile_y = face_center_y + 40
    draw.arc(
        [(face_center_x - 50, smile_y - 30), (face_center_x + 50, smile_y + 30)],
        start=0,
        end=180,
        fill=(255, 255, 255),
        width=4,
    ...
    # Checkmark
    draw.line(
        [(badge_x - 15, badge_y), (badge_x - 2, badge_y + 15), (badge_x + 20, badge_y - 15)],
        fill=(255, 255, 255),
        width=6,
    )
