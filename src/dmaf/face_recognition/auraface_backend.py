# auraface_backend.py (AuraFace backend - Apache 2.0 licensed)
import logging
import threading
...
# So we use ~/.dmaf as root and download to ~/.dmaf/models/auraface/
MODEL_ROOT = Path.home() / ".dmaf"
MODEL_PATH = MODEL_ROOT / "models" / "auraface"


def _ensure_model_downloaded() -> Path:
    """
    Download AuraFace model from HuggingFace if not already cached.

    AuraFace is Apache 2.0 licensed, enabling fully commercial use.
    Model is downloaded once to ~/.dmaf/models/auraface/ and reused.

    Returns:
        Path to the model root directory (for FaceAnalysis)
    """
    if not MODEL_PATH.exists() or not any(MODEL_PATH.iterdir()):
        logger.info(f"Downloading AuraFace model from HuggingFace ({MODEL_REPO})...")
        logger.info("This is a one-time download (~600MB, may take a few minutes)")

        try:
            from huggingface_hub import snapshot_download

            # Download to ~/.dmaf/models/auraface/
            snapshot_download(
                repo_id=MODEL_REPO,
                local_dir=str(MODEL_PATH),
                local_dir_use_symlinks=False,
            )
            logger.info(f"AuraFace model downloaded successfully to {MODEL_PATH}")
        except ImportError as e:
            raise ImportError(
                "huggingface_hub is required for AuraFace backend. "
                "Install with: pip install dmaf[auraface]"
            ) from e
    ...
    if det_thresh not in _app_cache:
        with _app_lock:
            ...
            if det_thresh not in _app_cache:
                ...
                app = FaceAnalysis(
                    name="auraface", root=str(model_path), providers=["CPUExecutionProvider"]
                )
                # ctx_id=-1 = CPU; set 0 if you configured CUDA
...
def best_match(
    ...
    """
    Find matching faces in an image using AuraFace.
    ...
    """
    app = _get_app(det_thresh=det_thresh)
    # AuraFace (like InsightFace) expects RGB input, which is what PIL provides
